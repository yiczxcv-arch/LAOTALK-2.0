...
import { inquiryRecords, reservationStatuses, type ReservationStatus } from "@/lib/data/admin";
...
/** 맞춤여행 문의 목록 (docs/02_BLUEPRINT.md #9 관리자 · design/wireframe 7번.png A5) */
export default function AdminInquiriesPage() {
  const [filter, setFilter] = useState<FilterValue>("all");

  const filtered = useMemo(
    () => (filter === "all" ? inquiryRecords : inquiryRecords.filter((i) => i.status === filter)),
    [filter],
  );

  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-title1 text-foreground">문의 관리</h1>
        ...
        {filtered.map((i) => (
          ...
          >
            <div className="flex min-w-0 flex-col gap-1">
              <span className="text-caption2 text-muted-foreground">{i.id}</span>
              <p className="truncate text-body1 text-foreground">
                {i.travelStyleLabel} · {i.regionLabel}
              </p>
              <p className="text-caption2 text-muted-foreground">
                {i.durationLabel} · {i.peopleCount}명
              </p>
            </div>
